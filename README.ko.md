...
`structural.NewLayerPlacement()`, `structural.NewAlias()`,
...
### `structural.domain-alias-exists` (DDD만)
...
### `structural.domain-model-required` (DDD만)
...
`interfaces.NewTooManyMethods()` 룰이 emit (`interfaces.NewPattern`과 별개). 기본 상한은 10이며 `interfaces.WithMaxMethods(n)`으로 변경. 모든 권장 번들에 기본 상한으로 포함됨.
...
테스트 파일이 가져야 할 제약을 강제합니다. 테스팅 관련 정책은 네이밍/구조 컨벤션과 직교하므로 별도 패키지에 둡니다.

### `testpolicy.no-handmock`

테스트 파일은 hand-rolled mock/fake/stub struct를 정의하면 안 됩니다.
mockery 같은 mock 생성기를 사용해 별도 mocks 패키지에서 import 하세요.

## 인터페이스 패턴 규칙

`interfaces.NewPattern()`, `interfaces.NewTooManyMethods()`,
`interfaces.NewContainer()`, `interfaces.NewCrossDomainAnonymous()`
...
`presets.RecommendedDDD()`에 Error 규칙으로 포함됩니다. `infra`, `adapter`,
`repository` 같은 infra 성격의 sublayer 아래 adapter 패키지에서 같은 패키지
concrete adapter 타입을 반환하는 package-level `NewXxx` 생성자를, 허용된 생성자
이름이 `New`일 때 위반으로 보고합니다.

```go
func NewOrderMemoryStore() *Store  // package/domain 맥락 반복
func New() *Store                  // 간결한 생성자 이름
```

허용 생성자 이름과 검사할 sublayer는 `naming.WithAllowedConstructorNames(...)`,
`naming.WithInfraSublayers(...)`로 조정할 수 있습니다.

## Orchestration 규칙

`structural.NewLogicBudget()`

`presets.RecommendedDDD()`에 Warning 규칙으로 포함됩니다. `internal/orchestration`
같은 설정된 orchestration 디렉터리 아래 패키지에서 함수의 branch 수, statement 수,
cyclomatic complexity가 설정된 예산을 넘으면 위반으로 보고합니다. 기본 예산은
`maxBranches=8`, `maxStatements=40`, `maxCyclomatic=10`입니다.

기본값은 단순 `if err != nil { return err }` 및 `fmt.Errorf("%w", err)`
분기를 할인합니다. Go의 명시적 에러 흐름 때문에 점수가 부풀어 오르는 것을 줄이고,
orchestration 함수가 실제 비즈니스 의사결정이나 과도한 조정 로직을 품는 신호에
집중하기 위해서입니다. `if err := call(); err != nil { return err }`에서는
분기 자체는 할인하지만 `call()` init statement는 계속 statement로 계산합니다.
트랜잭션, 재시도, 콜백에 넘기는 function literal body도 enclosing function의
예산에 합산합니다.
...
더 엄격하게 계산하려면 `structural.WithCountErrorBranches()`를 쓰고,
예외 함수는 `structural.WithIgnoredFunctions(...)`, 별도 정책으로 관리할
transport handler 같은 하위 트리는 `structural.WithIgnoredPaths(...)`로
지정할 수 있습니다.
...
| 규칙 | 의미 |
|------|------|
| `dependency.high-coupling` | transitive dependents가 통계적 이상치 |
...
`presets.RecommendedDDD()`에 Error 규칙으로 포함됩니다. 도메인/애플리케이션
레이어에서 error 반환 경계를 우회하고 제어 흐름을 직접 종료하는 호출을 검출합니다.
...
`presets.RecommendedDDD()`에 Error 규칙으로 포함됩니다. `core/model`, `event`,
`entity` 또는 `Architecture.Layers.PkgRestricted`에 설정된 도메인 내부 레이어에서
런타임 부수효과 API를 직접 호출하면 위반으로 보고합니다. import 기반이 아니라
call 기반 검사이므로 `time.Time` 같은
...
| `naming.NewConstructorName()` | infra adapter 생성자 네이밍 규칙 (RecommendedDDD: Error) |
| `structural.NewLogicBudget()` | orchestration 복잡도 예산 규칙 (RecommendedDDD: Warning) |
| `types.NewNoPanicInDomain()` | domain/application 레이어의 panic, log.Fatal, os.Exit 검사 (RecommendedDDD: Error) |
| `types.WithInspectedLayers(...)` / `WithAllowedPaths(...)` / `WithAllowedFunctions(...)` | `types.NewNoPanicInDomain` 옵션 |
| `dependency.NewNoSideEffectCallInCore()` | 도메인 코어 부수효과 호출 검사 (RecommendedDDD: Error) |
