...
### Composition Root Infra Import (opt-in)
...
필요하면 `analyzer.LoadWithTests(...)`와 `dependency.WithTestFiles(true)`를
함께 사용해 포함한다.
...
| 룰 | 잡는 패턴 |
|----|----------|
| `composition.root-only-infra-use` | handler, orchestration, domain subpackage, server 등이 domain `infra` adapter를 직접 import하는 패턴. concrete adapter 선택은 `internal/app` 또는 `cmd/...` 같은 composition root가 소유해야 한다. |
| `domain.non-empty-alias` | app package가 있는 도메인의 root `alias.go`가 `package <domain>`만 있고 exported surface가 없는 패턴. alias-only import boundary가 실제로 쓸 수 있는 surface를 제공하는지 확인한다. |
