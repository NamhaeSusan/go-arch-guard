...
| `setter.forbidden` | 포인터 리시버를 가진 내보내기 세터 메서드(`Set*`, 매개변수 1개 이상)를 검출. 생성자 또는 `With*` 옵션 패턴으로 대체를 유도한다. 플루언트 빌더(리시버 타입 반환 메서드), 테스트 파일, `testdata/`·`mocks/` 하위 패키지는 자동 제외. `CheckNoSetters(pkgs, opts...)`. |
