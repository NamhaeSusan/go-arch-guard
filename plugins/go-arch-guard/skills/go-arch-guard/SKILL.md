...
### Handler Response DTO Boundary (opt-in)

핸들러/transport 응답 경계가 domain model을 API 계약으로 그대로 노출하지 않게
검사한다. 권장 번들에는 포함하지 않고, 팀이 DTO 경계를 엄격히 보고 싶을 때만
명시적으로 추가한다.
...
검사 대상:
- `internal/domain/<name>/handler/...`
- `internal/orchestration/handler/...`
...
- app-level DTO 자체
- `handler.WithAllowedModelTypes(...)`로 명시 허용한 model type

위반 규칙 ID: `handler.no-model-response`.
...
| `handler.no-model-response` | handler/transport 응답 타입이나 exported handler result가 `core/model`, `entity` 같은 configured domain model 타입을 그대로 노출하는 패턴을 잡는다. 기본 Warning, opt-in. request DTO와 parameter는 제외. |
