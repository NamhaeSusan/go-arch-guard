...
`dependency.WithAllowedCalls(...)`로 검사 레이어와 호출 deny/allow 목록을 조정한다.

위반 규칙 ID: `purity.no-side-effect-call-in-core`.
...
| 룰 | 잡는 패턴 |
|----|----------|
| `errors.no-panic-in-domain` | 도메인/애플리케이션 레이어의 `panic`, `log.Fatal*`, `os.Exit`. 도메인은 error를 반환하고 복구/재시도/롤백/종료 정책은 바깥 레이어가 소유해야 한다. `types.NewNoPanicInDomain()`. |
| `purity.no-side-effect-call-in-core` | 도메인 내부 레이어에서 `time.Now`, `os.Getenv`, 파일 I/O, logging, random, `net/http` shortcut 같은 런타임 부수효과 API를 직접 호출하는 패턴. 런타임 값은 바깥 레이어에서 명시적으로 주입한다. `dependency.NewNoSideEffectCallInCore()`. |
