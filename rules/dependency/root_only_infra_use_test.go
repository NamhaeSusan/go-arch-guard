...
func TestRootOnlyInfraUseModuleMismatchEmitsMetaWarning(t *testing.T) {
	module := "example.com/shop"
	root := writeFixture(t, module, map[string]string{
		"internal/domain/order/infra/memory/store.go": `package memory
...
`,
	...
	violations := dependency.NewRootOnlyInfraUse().Check(ctx)

	assertViolationCount(t, violations, "meta.no-matching-packages", 1)
	for _, v := range violations {
		if v.Rule == rootOnlyInfraUseRule {
			t.Fatalf("unexpected %s violation: %+v", rootOnlyInfraUseRule, v)
		}
	}
}
...
func TestRootOnlyInfraUseDeduplicatesPackagesLoadedWithTests(t *testing.T) {
	module := "example.com/shop"
	root := writeFixture(t, module, map[string]string{
		"internal/domain/order/infra/memory/store.go": `package memory
...
`,
	...
	violations := dependency.NewRootOnlyInfraUse(dependency.WithTestFiles(true)).
		Check(loadRootOnlyInfraContext(t, root, module, true))

	assertViolationCount(t, violations, rootOnlyInfraUseRule, 2)
	assertViolationAtCount(t, violations, "internal/domain/order/handler/http/handler.go", 1)
	assertViolationAtCount(t, violations, "internal/domain/order/handler/http/handler_test.go", 1)
}
...
func loadRootOnlyInfraContext(t *testing.T, root, module string, includeTests bool) *core.Context {
	...
	var err error
	if includeTests {
		pkgs, err = analyzer.LoadWithTests(root, loadPatterns(root)...)
	} else {
		pkgs, err = analyzer.Load(root, loadPatterns(root)...)
	}
	if err != nil {
...
func assertViolationAtCount(t *testing.T, violations []core.Violation, file string, want int) {
	t.Helper()
	...
	if got != want {
		t.Fatalf("%s violation count at %s = %d, want %d; got %+v", rootOnlyInfraUseRule, file, got, want, violations)
	}
}
