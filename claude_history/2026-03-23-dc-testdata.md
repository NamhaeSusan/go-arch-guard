...
- Full domain-centric structure with user/order domains, orchestration, and pkg
- Proper alias pattern (root package re-exports app types)
- Correct layer dependencies (handler->app, infra->core, svc->model only)
- Cross-domain communication via orchestration using root aliases
...
- HARD: orchestration imports user/core/model directly (bypasses alias)
