...
By default `_test.go` files are skipped. To check tests, load packages with
`analyzer.LoadWithTests(...)` and enable `dependency.WithTestFiles(true)`.
...
| `analyzer.LoadWithTests(dir, patterns...)` | load Go packages including test variants for rules that inspect `_test.go` files |
